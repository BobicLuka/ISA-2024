...
public class ComplaintService {
	...
	public Complaint create(ComplaintDTO complaintDTO) {
		...
		if (admin != null) { // zalbe za admina uslovi
			...
			if (!appointmentService.hadAppointmentWithAdmin(complainant.getId(), admin.getId())) { // morao je imati
																									// termin kod admina
																									// da bi se zalio
				return null;
			}
		} else { // zalbe za company
			if (!appointmentService.hadAppointmentWithCompany(complainant.getId(), company.getId())) { // morao je imati
																										// termin kod
																										// admina da bi
																										// se zalio
				return null;
